...
{
    private string audioPath = Path.Combine(Paths.Exiled, "Audio", "CommonUtils");
    ...
    [Description("Full path to the directory containing audio files. Defaults to <EXILED>/Audio/CommonUtils if empty or omitted.")]
    public string AudioPath
    {
        ...
        set
        {
            if (value.IsEmpty())
            {
                audioPath = Path.Combine(Paths.Exiled, "Audio", "CommonUtils");
            }
            else
            {
                audioPath = value;
